...
## 🧠 2 - Explanation
Regarding the Viewer (animate_cnc.py), simply install the necessary modules via PIP and run the command from an SVG generated by GCMC.

Each Layer can be a process; for the lines belonging to a layer to be executed, there needs to be a Tool drawn at coordinates (0,0) and the stroke needs to have an opacity of 0, so that the viewer considers it as a tool and the layer can be executed.

pip install svgpathtools drawsvg cairosvg imageio[ffmpeg] Pillow tqdm

Cairosvg requires the c libraries; on Ubuntu, use this command to install them before installing the module.
...
Sobre o Visualizador (animate_cnc.py), basta instalar os modulos necessarios via PIP e rodar o comando a partir de um SVG gerado pelo GCMC.
